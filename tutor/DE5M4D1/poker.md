## Setup

**Introduce the concept - don't assume elarners know about Plannng Poker**

- https://poker.qaalabs.com/de5m4 ~ For learners
- https://poker.qaalabs.com/de5m4/poker ~ Tutor page

*Click the link to see example User Stories that can be estimated by the group*

---

- `0`    - **No effort** ~ already done, or negligible
- `0.5`  - **Trivial** ~ tiny fix or tweak
- `1-13` - **Core Fibonacci** ~ realistic, small-to-medium stories
- `20`   - **Larger** ~ possibly needs breaking down
- `40`   - **Much larger** ~ rarely used, red flag
- `100`  - **Massive** ~ usually too big, should be split

- ✋ - **Hand-up** ~ signals that you need more information
- ❓ - **Not Sure** ~ signals uncertainty
- ☕ - **Break** ~ signals you will not be voting

---

### Main Links
- https://poker.qaalabs.com/de5m4
- https://poker.qaalabs.com/de5m4/poker

### Health check
- https://poker.qaalabs.com/health

### API list of rooms
- https://poker.qaalabs.com/api/rooms
